 ...
 */
public class Constant extends AbstractArchive implements Completable, Createable, Linkable, Modifiable, Deletable {
    ...
    public ConstantEntity getEntity() {
    ...
    /**
     * 构造函数，已过时，请使用Constant.build(blackboard).forComplete(id)
     *
     */
    
    public  Constant(Long id) {
        logger.info(this + "#Constant");
        Assert.notNull(id);
        entity = new ConstantEntity();
        entity.setId(id);
    }

    /**
     * 构造函数，已过时，请使用Constant.build(blackboard).forCreate(entity,)
     *
     */
    
    public  Constant(ConstantEntity entity) {
        logger.info(this + "#Constant");
    ...
    public void complete(boolean allowDelete) {
        ...
        ConstantEntity entity = dao.findOne(this.entity);
        if (entity == null)
            return;
        this.entity = entity;
     ...
     *
     */
    public Constant forComplete(Long id) {
        logger.info(this + "#forComplete");
        Constant build = new Constant(id);
     ...
     *
     */
    public Constant forCreate(ConstantEntity entity) {
        logger.info(this + "#forCreate");
        Constant build = new Constant(entity);
        build.setBlackboard(blackboard);
        return build;
    }
