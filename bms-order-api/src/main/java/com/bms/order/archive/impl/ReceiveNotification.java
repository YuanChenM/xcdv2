 ...
 */
public class ReceiveNotification extends AbstractArchive implements Completable, Createable, Linkable, Modifiable, Deletable {

    private static Logger logger = LoggerFactory.getLogger(ReceiveNotification.class);
    ...
    /** 收货通知单Entity */
    private ReceiveNotificationEntity entity;

    public ReceiveNotificationEntity getEntity() {
        logger.info(this + "#getEntity");
        return entity;
    }

    /** 收货通知单履历表Entity */
    private ReceiveNotificationHistoryEntity historyEntity;

    public ReceiveNotificationHistoryEntity getHistoryEntity() {
        ...
        if (receiveNotificationDetails == null || !isReceiveNotificationDetailsComplete) {
            ...
            if(list.isDataSynced()) {
                ...
                for (ReceiveNotificationDetail item : receiveNotificationDetails)
                    item.receiveNotificationLinked();
            }
        }
    ...
    /**
     * 构造函数，已过时，请使用ReceiveNotification.build(blackboard).forComplete(id)
     *
     */
    
    public  ReceiveNotification(Long id) {
        logger.info(this + "#ReceiveNotification");
        Assert.notNull(id);
        entity = new ReceiveNotificationEntity();
        entity.setReceiveNotificationId(id);
    }

    /**
     * 构造函数，已过时，请使用ReceiveNotification.build(blackboard).forCreate(entity,)
     *
     */
    
    public  ReceiveNotification(ReceiveNotificationEntity entity) {
        logger.info(this + "#ReceiveNotification");
    ...
    /**
     * 构造函数，已过时，请使用ReceiveNotification.build(blackboard).forComplete(linkedArchive)
     *
     */
    
    public  ReceiveNotification(DistributionNotification linkedArchive) {
        logger.info(this + "#ReceiveNotification");
    ...
    public void complete(boolean allowDelete) {
        ...
        if (entity.getReceiveNotificationId() != null) {
            ReceiveNotificationEntity entity = new ReceiveNotificationEntity();
        ...
        ReceiveNotificationEntity entity = dao.findOne(this.entity);
        if (entity == null)
            return;
        this.entity = entity;
    ...
    public void create() {
        ...
        // 设置ID
        this.entity.setReceiveNotificationId(id);
    ...
    public void setHistory() {
        ...
        Assert.notNull(entity);
        historyEntity = new ReceiveNotificationHistoryEntity(entity);
    }
    ...
    public String toString() {
        if (entity != null)
            return this.getClass().getName() + this.entity.getReceiveNotificationId();
        else
     ...
     *
     */
    public ReceiveNotification forComplete(Long id) {
        logger.info(this + "#forComplete");
        ReceiveNotification build = new ReceiveNotification(id);
     ...
     *
     */
    public ReceiveNotification forCreate(ReceiveNotificationEntity entity) {
        logger.info(this + "#forCreate");
        ReceiveNotification build = new ReceiveNotification(entity);
        build.setBlackboard(blackboard);
        return build;
    }
     ...
     *
     */
    public ReceiveNotification forComplete(DistributionNotification linkedArchive) {
        logger.info(this + "#forComplete");
        ReceiveNotification build = new ReceiveNotification(linkedArchive);
        build.setBlackboard(blackboard);
        return build;
    }
