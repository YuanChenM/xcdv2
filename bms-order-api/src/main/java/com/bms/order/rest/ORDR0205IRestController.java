...
import com.bms.order.validator.ORDR0205IRestValidator;
...
/**
 * 订单手动分拆阈值设定接口Controller
 *
 * @author li_huiqian
 */
@RestController
@Api(tags = { "ORDR0205IRestController" }, description = "订单手动分拆阈值设定接口")
public class ORDR0205IRestController extends BaseRestController {

    @Autowired
    private ORDR0205IService service;
     ...
     */
    @ApiOperation(value = "订单手动分拆阈值设定接口", notes = "设定是否进行手动分拆")
    @Validator(validatorClass = ORDR0205IRestValidator.class)
    @RequestMapping(value = "/order/split/settings/_set", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public BusinessRestResult execute(@RequestBody BusinessRestParam<ORDR0205IBeanParam> param) {
        ...
        for (int i = 0; i < beans.size(); i++) {
            ORDR0205IBeanParam bean = beans.get(i);
            try {
                service.execute(operator, bean);
                succeedCount++;
                RestSucceedData succeedData = new RestSucceedData();
                succeedData.setSucceedId(bean.getBuyerBatchOrderDetailId());
