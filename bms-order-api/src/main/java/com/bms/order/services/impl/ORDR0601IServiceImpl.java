 ...
 */
@Service
public class ORDR0601IServiceImpl implements ORDR0601IService {
     ...
     *
     * @param operator 操作者
     * @param param    ORDR0601IRestParam
     */
    @Transactional
    @Override
    public Long execute(Operator operator, ORDR0601IBeanParam param) {
        // 本Service用参数传递容器
    ...
    private void pre(HashMap<String, Object> map) {
        // 获取参数
        ORDR0601IBeanParam param = (ORDR0601IBeanParam) map.get("param");
    ...
    private void process(HashMap<String, Object> map) {
        ...
        // 设置区域信息
        setLogistics(bsOrder, bsEntity, param);
     ...
     * @param bs
     */
    private void setLogistics (BsOrder bsOrder, Bs bs, ORDR0601IBeanParam param) {
        // 设置区域信息
        if (!StringUtils.isEmpty(param.getLgcsAreaCode())){
            REGN0102IRestResult regn0102IRestResult = Logistics.findLogisticsByCode(param.getLgcsAreaCode());
            bsOrder.getEntity().setLogisticsZoneId(regn0102IRestResult.getLgcsAreaId());
            bsOrder.getAttachedEntity().setLogisticsZoneCode(regn0102IRestResult.getLgcsAreaCode());
            bsOrder.getAttachedEntity().setLogisticsZoneName(regn0102IRestResult.getLgcsAreaName());
        } else {
