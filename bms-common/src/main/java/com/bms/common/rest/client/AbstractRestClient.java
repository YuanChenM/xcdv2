...
import com.bms.common.param.User;
...
public abstract class AbstractRestClient {
    ...
    protected AbstractRestClient(int connectTimeout, int readTimeout, int asyncConnectTimeout, int asyncReadTimeout, String apiKey, String securityKey) {
    ...
    private HttpHeaders createRequestHeaders(String[] showField, String... hideField) {
        ...
        Object ssoUser = httpSession.getAttribute("loginUser");
        Assert.notNull(ssoUser);
        Date date = new Date();
        String timestamp = String.valueOf(date.getTime());

        User user = BeanUtils.toBean(ssoUser, User.class);
        headers.add("sign", user.getUserId());
        headers.add("api_key", this.apiKey);
        headers.add("timestamp", timestamp);
        headers.add("security_key", Md5RestDigest.digest(user.getUserId(), this.securityKey, timestamp));
