...
import com.bms.issue.validator.ISSU0205IRestValidator;
 ...
 */
@RestController
@Api( tags = {"ISSU0205IRestController"}, description = "问题附件删除接口")
public class ISSU0205IRestController extends BaseRestController {

    @Autowired
    private ISSU0205IService service;
     ...
     */
    @ApiOperation(value = "问题附件删除接口", notes = "接收问题的附件信息，删除附件信息，即更新删除标志")
    @Validator(validatorClass = ISSU0205IRestValidator.class)
    @RequestMapping(value = "/issue/question/accessory/_del", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public BusinessRestResult execute(@RequestBody BusinessRestParam<ISSU0205IBean> param) {
        List<ISSU0205IBean> beans = param.getBeans();
        ...
        for (int i = 0; i < beans.size(); i++) {
            ISSU0205IBean bean = beans.get(i);
            try {
                ...
                failedData.setFailedDataId(bean.getAccessoryId());
